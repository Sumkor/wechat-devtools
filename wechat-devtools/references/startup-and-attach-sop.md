...
- 当前平台存在已验证的项目 CLI、IDE 已关闭、目标已导入：只执行一次该 CLI 的项目打开命令，记录实际入口和 `launchMode: cli-automation`。
- 没有已验证项目 CLI 或用户要求普通 GUI：无参数启动桌面程序，记录 `launchMode: normal-gui`；普通 GUI 可能自动恢复上次项目。
